<script lang="ts">
    ...
    // Mantener el rango de fechas seleccionado por el usuario
    ...
    // Variable para seguir la fecha sobre la que está el cursor durante la selección
                ...
                <div class="flex items-center justify-between w-full">
                    <div class="flex-1 flex items-center">
                        <span class={!selectedRange?.start ? "text-muted-foreground" : ""}>
                            {selectedRange?.start ? `${selectedRange.start.day}/${selectedRange.start.month}/${selectedRange.start.year}` : "Fecha inicio"}
                        </span>
                        <div aria-hidden="true" class="text-muted-foreground px-1">–⁠⁠⁠⁠⁠</div>
                        <span class={!selectedRange?.end ? "text-muted-foreground" : ""}>
                            {selectedRange?.end ? `${selectedRange.end.day}/${selectedRange.end.month}/${selectedRange.end.year}` : "Fecha fin"}
                        </span>
                    </div>
                    <button 
                        type="button" 
